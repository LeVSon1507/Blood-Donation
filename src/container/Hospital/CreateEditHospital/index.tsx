...
const CreateEditHospital: React.FC = () => {
   ...
   const onSubmit = value => {
      setIsAdding(true);
      const bloodbankid = JSON.parse(localStorage.getItem('currentUser'))?.userId;

      const payload = { ...value, img: '', bloodbankid: bloodbankid };

      http
         .post('manager/createhospital', payload)
         .then(res => {
               ...
               <Grid xs={12}>
                  <Grid xs={6} mb={2} gap={1}>
                     <TextField
                        ...
                        {...register('nameHospital')}
                     />
                     {errors.nameHospital && (
                        <p className='text-sm text-red-500 color-red'>
                           {errors.nameHospital.message}
                        </p>
                     )}
                  </Grid>
                  <Grid xs={6} mb={2} gap={1}>
